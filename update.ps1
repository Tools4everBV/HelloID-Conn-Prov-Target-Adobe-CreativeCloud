...
$body += '&jwt_token=' + $config.jwt_token;
...
$user = @(@{
    'user' = $pp.emailAddress;
    'requestID' = 'HelloIDUpdate_' + $p.externalId;
    'do' = @(@{
        'update' = @{
...
try {
    ...
    $auditLogs.Add([PSCustomObject]@{
        ...
        Message = "Updated account for Email $($p.emailAddress)"
        IsError = $false;
    });
    $success = $True
}
catch
{
    $auditLogs.Add([PSCustomObject]@{
                Action = "UpdateAccount"
                Message = "Account failed to update:  $_"
                IsError = $True
            });
	Write-Error $_;
}